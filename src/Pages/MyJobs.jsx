...
const MyJobs = () => {
    ...
    useEffect(()=>{
        setIsLoading(true)
        fetch(`http://localhost:5000/myJobs/palak@gmail.com`).then((res)=>res.json()).then((data)=>{setJobs(data); setIsLoading(false);})
    }, [searchText]);
            ...
            isLoading ? (<div className='flex items-center justify-center h-20'><p>loading.....</p></div>):(<tbody>
                { 
                    currentJobs.map((job, index)=>(
                        ...
                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                        <button onClick={()=> handleDelete(job._id)} className='bg-red-700 py-2 px-6'>Delete</button>
                        </td>
                    </tr>
                    ))
                }
